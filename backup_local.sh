...
if [ "${1}x" == "homex" ]; then
	...
	if [ "${HOSTNAME}" == "mars" ]; then
		tar cvfJ ${TARBALL} --exclude /dev --exclude /tmp --exclude /proc --exclude /sys --exclude-vcs --exclude-backups --exclude /media --exclude /mnt --exclude /var/tmp --exclude /run --exclude /var/run --exclude "swe*" --exclude "otw*" --exclude "*.iso" --exclude "/home/*/VirtualBox VMs/" --exclude "*/vmware/*" --exclude /s /
	else
		tar cvfJ ${TARBALL} --exclude /dev --exclude /tmp --exclude /proc --exclude /sys --exclude-vcs --exclude-backups --exclude /media --exclude /mnt --exclude /var/tmp --exclude /run --exclude /var/run --exclude "swe*" --exclude "otw*" --exclude "*.iso" --exclude "/home/*/Virtualbox VMs/" --exclude "*/vmware/*" /
	fi
fi
